...
const NavItem = ({ label, href, icon, badge, active }: NavItemProps) => {
  ...
  return (
    <button 
      ...
      className={cn(
        "w-full flex items-center gap-[7px] px-2 py-[5px] rounded-[var(--r6)] cursor-pointer text-t3 text-[13px] font-medium relative select-none transition-all duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] hover:bg-s2 hover:text-t1 hover:translate-x-0.5 active:scale-[0.98]",
        active && "bg-acc2 text-acc hover:bg-acc2 hover:text-acc hover:translate-x-0"
      )}
    >
